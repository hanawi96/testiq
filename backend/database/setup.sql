...
-- =======================
-- 6. ANONYMOUS PLAYERS TABLE
-- =======================

-- Create anonymous_players table for IQ test results
...
-- Enable RLS on anonymous_players (publicly readable but only service role can insert)
alter table public.anonymous_players enable row level security;

-- Allow anyone to read anonymous player data
create policy "Anonymous players are publicly readable" on public.anonymous_players for select using (true);

-- Only service role can insert new anonymous players (from backend)
create policy "Service role can insert anonymous players" on public.anonymous_players for insert with check (true);

-- Add indexes for better query performance
create index if not exists idx_anonymous_players_created_at on public.anonymous_players(created_at desc);
create index if not exists idx_anonymous_players_test_score on public.anonymous_players(test_score desc);
create index if not exists idx_anonymous_players_location on public.anonymous_players(location);

-- Add comment
comment on table public.anonymous_players is 'Stores anonymous IQ test results with basic user information';
