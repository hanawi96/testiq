...
interface AvatarCropperProps {
  ...
  onCancel: () => void;
  onStartCrop?: () => void; // New callback for immediate loading
}

export default function AvatarCropper({ imageUrl, onCrop, onCancel, onStartCrop }: AvatarCropperProps) {
  ...
  const handleMouseDown = useCallback((e: React.MouseEvent) => {
    ...
    console.log('🖱️ Avatar Drag Start:', {
  ...
  useEffect(() => {
    const handleMouseMove = (e: MouseEvent) => {
      ...
      console.log('🖱️ Avatar Drag Move:', {
        mousePos: { x: e.clientX, y: e.clientY },
        imageRect: rect,
        dragStart: dragStart,
        newPos: { x: newX, y: newY },
        boundaries: { maxX: rect.width - cropArea.width, maxY: rect.height - cropArea.height }
      });
  ...
  }, [isDragging, isResizing, dragStart, cropArea.width, cropArea.height]);
  ...
  const handleCrop = useCallback(async () => {
    ...
    try {
      ...
      // Set canvas size for avatar (400x400)
      canvas.width = 400;
      canvas.height = 400;
      ...
      corsImage.onload = async () => {
        try {
          ...
          console.log('🎨 Avatar Canvas Crop Execution:');
          console.log('  📍 Crop Area on Display:', currentCropArea);
          console.log('  📏 Image Dimensions (Natural):', currentImageDimensions);
          console.log('  📐 Display Rect:', { width: imgRect.width, height: imgRect.height });
          console.log('  ⚖️ Scale Factors:', { x: scaleX, y: scaleY });
          console.log('  🎯 Source Coordinates:', { x: sourceX, y: sourceY, width: sourceWidth, height: sourceHeight });
          console.log('  🖼️ Canvas Output Size:', { width: 400, height: 400 });

          // Draw cropped image (square crop) with correct coordinates
          ctx.drawImage(
            corsImage,
            sourceX,
            sourceY,
          ...
          console.log('✅ Canvas drawImage completed');
          ...
          canvas.toBlob(async (blob) => {
            ...
            try {
              // Create blob URL directly (skip optimization for now)
              const croppedUrl = URL.createObjectURL(blob);
              console.log('✅ Cropped URL created:', croppedUrl.substring(0, 50) + '...');
  ...
  }, [imageUrl, cropArea, imageDimensions, onCrop, onCancel]);
        ...
        <div className="p-6">
          <div className="text-center mb-4">
            <p className="text-sm text-gray-600 dark:text-gray-400">
              Kéo vùng vuông để chọn phần ảnh bạn muốn làm ảnh đại diện
            </p>
          </div>
